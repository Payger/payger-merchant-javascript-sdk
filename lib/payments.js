...
export default class Payments {
	...
	getPayments(token, paginationOptions) {
		...
		return this.request.get(token, this.paymentsUrl + parameters);
	}
	...
	*/
	savePayment(token, payment){
		return this.request.post(token, this.paymentsUrl, payment);
	}
	...
	updatePayment(token, id, data){
		return this.request.put(token, this.paymentsUrl + "/" + id, data);
	}
	...
	addAddress(token, id, address){
		...
		return this.request.post(token, url, address);
	}
